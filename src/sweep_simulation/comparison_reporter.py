...
Classes:
    GroupComparator: Compares real vs simulated metrics by group
...
class GroupComparator:
    """
    Compares real execution with simulated execution by order group.
    ...
    def compare_by_group(
        self,
        orders_with_metrics: pd.DataFrame,
        ...
        """
        logger.info("Comparing real vs simulated execution across groups")
        ...
        for group_name, group_orders in groups.items():
            logger.debug(f"Comparing group {group_name}: {len(group_orders)} orders")
            
            # Get orders with metrics for this group
            # Handle both order_id and orderid column names
            ...
            if len(group_with_metrics) == 0:
                logger.warning(f"No orders found for group {group_name}")
                continue
            
            # Calculate group-level statistics
            summary = self._calculate_group_summary(group_name, group_with_metrics)
            group_summaries.append(summary)
            
            # Calculate order-level details
            order_details = self._calculate_order_details(group_name, group_with_metrics)
            all_order_details.extend(order_details)
            
            # Detailed group analysis
            analysis = self._analyze_group_differences(group_name, group_with_metrics)
    ...
    def _calculate_group_summary(
        ...
        Args:
            ...
            group_df: DataFrame with orders and metrics for this group
    ...
    def _calculate_order_details(
        ...
        Args:
            group_name: Name of the group
            group_df: DataFrame with orders and metrics
        
        Returns:
            List of dictionaries with order-level details
        """
        details = []
    ...
    def _analyze_group_differences(
        self,
        group_name: str,
        group_df: pd.DataFrame
    ) -> Dict[str, Any]:
...
class ComparisonReporter:
    """
    Generates comparison reports between real and simulated execution.
    """
    ...
    def _generate_statistical_summary(
        ...
        Args:
            comparison_data: Dictionary containing comparison DataFrames
        
        Returns:
            Path to statistical summary file
        """
        stats = []
        ...
        stats_file = self.output_dir / 'statistical_summary.csv'
        stats_df.to_csv(stats_file, index=False)
        logger.debug(f"Wrote statistical summary: {stats_file}")
