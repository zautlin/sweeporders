...
def main():
    ...
    logger.info("    - centrepoint_orders_raw.parquet")
    logger.info("    - centrepoint_trades_raw.parquet")
    logger.info("    - centrepoint_trades_agg.parquet")
    logger.info("    - dark_book_state.pkl")
    logger.info("    - order_index.pkl")
    logger.info("  Scenario classification:")
    logger.info("    - scenario_a_immediate_full.parquet")
    logger.info("    - scenario_b_eventual_full.parquet")
    logger.info("    - scenario_c_partial_none.parquet")
    logger.info("  Simulation results:")
    logger.info("    - scenario_a_simulation_results.parquet")
    logger.info("    - scenario_b_simulation_results.parquet")
    logger.info("    - scenario_c_simulation_results.parquet")
    logger.info("  Reports (CSV):")
    logger.info("    - scenario_comparison_summary.csv")
    logger.info("    - scenario_detailed_comparison.csv")
    logger.info("    - order_level_detail.csv")
    logger.info("    - execution_cost_comparison.csv")
    logger.info("    - by_participant.csv")
    logger.info("=" * 80)
